...
<a href="${pageContext.request.contextPath }/storyBoard.report?idx=${detail.idx}&title=${detail.title}"><img src="${pageContext.request.contextPath}/resources/assets/images/6.PNG" alt="Post Image" /></a>
</div>
</div>
<!-- 신고 이미지 -->
<hr>
...
<button type="button"  class="btn btn-default" onclick="location.href='${pageContext.request.contextPath}/storyBoard.list'" value="목록">목록</button>
...
<button type="button" id="delete" class="btn btn-default" value="삭제">삭제</button>
						...
						<a href="${pageContext.request.contextPath }/storyBoard.commentReport?board_idx=${best.board_idx}&comment_idx=${best.idx}&content=${best.content}"><i class="fa fa-bell-o" aria-hidden="true fa-lg"></i>신고</a>&nbsp;&nbsp;&nbsp;
						...
						<hr>
								...
								<c:if test="${comment.indent==0}">
										<div class="comment-body">
											...
											<c:if test="${comment.available==0}">
												...
												<a href="${pageContext.request.contextPath }/storyBoard.commentReport?board_idx=${detail.idx}&comment_idx=${comment.idx}&content=${comment.content}"><i class="fa fa-bell-o" aria-hidden="true fa-lg"></i>신고</a>&nbsp;&nbsp;&nbsp;
											</c:if>
									<!-- 삭제 -->
											<c:if test="${comment.available==0}">
											<a class="commentDelete" href="${pageContext.request.contextPath}/storyBoard.commentDelete?idx=${comment.idx}&board_idx=${detail.idx}"><i class="fa fa-trash-o fa-lg" aria-hidden="true"></i>삭제<input type="hidden" value="${comment.idx }"></a>&nbsp;&nbsp;&nbsp;
											</c:if>
									<!-- 삭제 -->	
										<!-- 댓글수정 -->
										<c:if test="${comment.available==0}">
											<i class="fa fa-pencil-square-o" aria-hidden="true"></i>&nbsp;<a  href='javascript:void(0)' onclick="this.innerHTML=(this.nextSibling.style.display=='none')?'취소':'댓글수정';this.nextSibling.style.display=(this.nextSibling.style.display=='none')?'block':'none'">댓글수정</a><DIV style='display:none'>				
												<div class="comment-respond">
													...
													<form method="post" class="comment-form" action="${pageContext.request.contextPath}/storyBoard.commentModify">
							...
							&nbsp;&nbsp;<i class="fa fa-commenting-o" aria-hidden="true"></i>&nbsp;<a href='javascript:void(0)' onclick="this.innerHTML=(this.nextSibling.style.display=='none')?'취소':'댓글달기';this.nextSibling.style.display=(this.nextSibling.style.display=='none')?'block':'none'">댓글달기</a><DIV style='display:none'>				
								<div class="comment-respond">
									...
									<form method="post" class="comment-form" action="${pageContext.request.contextPath}/storyBoard.reCommentInsert">
										<p class="comment-form-author">
										...
										<ol class="children">
											<li class="comment byuser comment-author-admin bypostauthor odd alt depth-2 parent">
												<div class="comment-body">
													<footer class="comment-meta">
														<img src="${pageContext.request.contextPath}/resources/assets/images/567.PNG"><div class="comment-author vcard">
		...
		<!-- 												댓글수정 -->
                        ...
                        <div class="comment-respond">
                           <h2 class="comment-reply-title">댓글 쓰기</h2>
                           <form method="post" class="comment-form" action="${pageContext.request.contextPath}/storyBoard.commentInsert">
                              <p class="comment-form-author">
                                 <input type="hidden" name="board_idx" value="${detail.idx }">
...
$(document).ready(function(){
	...
	$('#modify').on('click',function(){
		var id = "${sessionId}";
		var locationPath="${pageContext.request.contextPath}/storyBoard.modify?idx=${param.idx}";
		var idx = ${detail.idx };
		var permission = "${sessionPermission}";
		if(id==""){		
			alert("로그인을 해주세요");			
	...
	$('#delete').on('click',function(){
		var id = "${sessionId}";
		var permission = "${sessionPermission}";
		var locationPath = "${pageContext.request.contextPath}/storyBoard.delet?idx=${param.idx}";
		var idx = ${detail.idx };
		if(id==""){		
			alert("로그인을 해주세요");			
		}else if(permission=="0"){
	...
	$('.commentDelete').on('click',function(e){
		...
		var comment_idx = $(this).children(':hidden').val();		
		var permission = "${sessionPermission}";
		var locationPath = "${pageContext.request.contextPath}/storyBoard.commentDelete?idx="+comment_idx+"&board_idx=${detail.idx}";
		if(id==""){
			alert("로그인을 해주세요");			
		}else if(permission=="0"){		
			location.href=locationPath;			
...
function recommend(recommend,idx){
	$.ajax({
		...
		url:"<%= request.getContextPath()%>/storyBoard.recommend",
...
function commentRecommend(recommend,idx,goodLocation,badLocation){
	$.ajax({
		type:"post",
		data:{"recommend":recommend,"idx":idx},
		url:"<%= request.getContextPath()%>/storyBoard.commentRecommend",
		success:function(result){			
...
function checkWriter(idx,locationPath){
	$.ajax({
		type:"post",
		data:{"idx":idx},
		url: "${pageContext.request.contextPath}/storyBoard.checkWriter",
		success:function(result){			
			if(result==1){				
				location.href=locationPath;
			}else{
...
function checkCommentWriter(idx,locationPath){
	$.ajax({
		type:"post",
		data:{"idx":idx},
		url:"${pageContext.request.contextPath}/storyBoard.checkCommentWriter",
		success:function(result){
			if(result==1){
				location.href= locationPath ;
			}else{
