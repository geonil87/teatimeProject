...
<body class="singletemplate" data-offset="200" data-spy="scroll" data-target=".ownavigation">
	...
	<div class="main-container">
					...
					<div class="slider-carousel-5">
						...
						<c:if test="${status.index==0 }">
							<div class="post-item active">
								<img src="${bestlist.photo}" alt="Slider" style="height: 500px; width: 1170px;"/>
								<div class="carousel-caption">
									
									<h3><a href="${pageContext.request.contextPath}/photoBoard.sumnail?idx=${bestlist.idx}" title="${bestlist.title}">${bestlist.title}</a></h3>
									<a href="${pageContext.request.contextPath}/photoBoard.sumnail?idx=${bestlist.idx}" title="Read More">더보기</a>
								</div>
							</div>
						</c:if>
						<c:if test="${status.index!=0 }">
				...
				<div class="container">
					<div class="row">
						...
						<div class="col-lg-12 col-md-12 content-area">
							...
							<!-- 썸네일 추가 박스 no.1 -->	
							<c:forEach items="${allPhotoboardlist}" var="photolist">														
								<div class="col-lg-4 col-md-6 col-sm-6">
									<div class="type-post">
										<div class="entry-cover">
											<div class="post-meta">
									
												<span class="byline"><a title="Indesign" style="text-transform:lowercase"><i class="fa fa-user-o" aria-hidden="true"></i> by ${photolist.user_nick}</a></span>
												<span class="post-date"><a>${photolist.wdate}</a></span>
											</div>
											<a href="${pageContext.request.contextPath}/photoBoard.sumnail?idx=${photolist.idx}"><img src="${photolist.photo}" alt="Post" style="height: 250px; width: 350px" /></a>
										</div>
										<div class="entry-content">
											<div class="entry-header">	
												
								<h4 class="entry-title" style="font-size: 19px;">
								<a title="Traffic Jams Solved">${photolist.title}</a></h4>
											</div>								
											<%-- <p style="font-size: 11px;">${photolist.text}</p> --%>
											<a href="${pageContext.request.contextPath}/photoBoard.sumnail?idx=${photolist.idx}" title="Read More">더보기</a>
										</div>
									</div>
								</div>
				<!-- 썸네일 추가 박스 no.1 -->	
			...
			<div class="container-fluid no-left-padding no-right-padding subscribe-block">
				<div class="container" >
					...
					<form class="newsletter" action="${pageContext.request.contextPath}/photoBoard.search">
						<div class="input-group">										
	...
	<option value="작성자">&nbsp;&nbsp;작성자</option>
						...
						<form action="${pageContext.request.contextPath }/photoBoard.insert" method="get">
						<!-- <input type="button" id="write"  class="btn btn-secondary" value="글쓰기"> -->
						<button type="button" class="btn btn-default" id="write" value="글쓰기">글쓰기</button>
							...
							<nav class="navigation pagination">
								...
								<div class="nav-links">
									...
									<input type="button" id="pageNum" class="page-numbers" value="${i }" onclick="location.href='<%=request.getContextPath()%>/photoBoard.list?pageNum=${i}'">
							</c:forEach>
...
$(document).ready(function(){
	$('#write').on('click',function(e){	
		...
		if(id==""){		
			...
			location.href="${pageContext.request.contextPath}/photoBoard.insert";
		} 
	});
	...
	$('#next').on('click',function(e){
		...
		if(finalPageNum < pageNum){
			...
			location.href="${pageContext.request.contextPath}/photoBoard.list?pageNum="+pageNum; 
		}		
	});
